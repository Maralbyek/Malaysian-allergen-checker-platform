...
export default function AddNewFoodPage() {
  ...
  // Fetch vendor profile to check status
  const { data: vendorProfile } = trpc.vendor.getProfile.useQuery()

  // Create dish mutation
  const createDishMutation = trpc.vendor.createDish.useMutation({
    onSuccess: () => {
      toast.success("Dish submitted for review successfully!")
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          >
            Create a new menu item with allergen information
          </motion.p>
        </div>
      </div>
          ...
          <Alert className="border-accent/50 bg-accent/5">
            ...
            <AlertDescription>
              {vendorProfile.status === 'PENDING' && (
                <>
                  Your vendor profile is pending approval. You can add dishes now, and they will be reviewed along with your profile by our admin team.
                </>
              )}
              {vendorProfile.status === 'REJECTED' && (
                <>
                  Your vendor profile was rejected. Please contact support before adding dishes.
                </>
              )}
            </AlertDescription>
          </Alert>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        >
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Select all allergens present in this dish. This information is critical for
                user safety.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
        ...
        >
          <Alert className="flex-1 mr-4">
            <Info className="h-4 w-4" />
            <AlertDescription>
              Your food item will be reviewed by our admin team before being published.
            </AlertDescription>
          </Alert>
          <div className="flex gap-3">
            ...
            <Button type="submit" disabled={isSubmitting || createDishMutation.isPending} className="gap-2">
